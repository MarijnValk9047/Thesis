...
from hydrogen.validation_cvar_expanded_sweep import (
    DEFAULT_ALPHA,
    DEFAULT_GAMMAS,
    DEFAULT_WEEK_IDS,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Phase D2 expanded validation-period CVaR sensitivity sweep.",
    )
    parser.add_argument("--config", required=True, help="Path to the hydrogen config YAML.")
    parser.add_argument("--week-ids", nargs="+", default=list(DEFAULT_WEEK_IDS), help="Selected validation regime labels, split-config labels, or canonical week IDs.")
    parser.add_argument("--artifacts", nargs="+", required=True, help="Scenario artifact IDs to run.")
    parser.add_argument("--alpha", type=float, default=float(DEFAULT_ALPHA), help="CVaR alpha. Phase D2 requires 0.95.")
    parser.add_argument("--gammas", nargs="+", type=float, default=list(DEFAULT_GAMMAS), help="Gamma grid. Phase D2 requires exactly 0 0.01 0.05 0.10 0.25 0.50.")
    parser.add_argument(
        "--include-price-insensitive-benchmark",
        type=_bool_arg,
        default=True,
        help="Whether to run the price-insensitive benchmark. Phase D2 requires true.",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    result = run_validation_cvar_expanded_sweep(
        config=config,
    ...
    print("gamma_policy_rows=", int(result.gamma_policy_candidate_summary.shape[0]))
