...
OUTPUT_PATH = OUTPUT_DIR / "nl_ir_capacity_milp_input_daily_direction_scenarios_v1.csv"

MARKET = "NL_incident_reserve"
PRODUCT = "mFRRda_capacity"
BIDDING_STAGE = "capacity_auction_D_minus_1_09am"
THRESHOLD_UNIT = "EUR_per_MW_per_period"
EXPORT_VERSION = "nl_ir_capacity_milp_input_daily_direction_scenarios_v1"
ACCEPTANCE_RULE = "bid_price_less_or_equal_acceptance_threshold_price"
REVENUE_RULE = "accepted_scenario_times_bid_price_times_offered_capacity_mw"
AVAILABILITY_NOTE = (
    "accepted_capacity_requires_deliverable_up_or_down_flexibility_across_contract_period_energy_activation_deferred"
)
...
OUTPUT_COLUMNS = [
    ...
    "acceptance_threshold_price_unit",
    "threshold_proxy_name",
    "threshold_model_name",
    "threshold_source_scope",
    "threshold_observed_overlap_flag",
...
def validate_repaired_timing(df: pd.DataFrame, label: str) -> None:
    for row in df.itertuples(index=False):
        ...
        parsed_origin_utc = datetime.fromisoformat(str(row.forecast_origin_utc).replace("Z", "+00:00"))
        ...
        if parsed_origin_utc != expected_utc:
            raise ValueError(f"{label}: forecast_origin_utc mismatch for {row.delivery_date_local} {row.direction}")
    ...
    export = pd.DataFrame(
        {
            ...
            "delivery_start_utc": merged["delivery_start_utc"],
            "delivery_end_utc": merged["delivery_end_utc"],
            ...
            "acceptance_threshold_price": merged["threshold_price_scenario"],
            "acceptance_threshold_price_unit": THRESHOLD_UNIT,
...
def validate_export(export: pd.DataFrame) -> dict[str, Any]:
    ...
    if sorted(export["activation_included"].dropna().unique().tolist()) != [False]:
        issues.append("activation_scope_mismatch")
    if sorted(export["imbalance_settlement_included"].dropna().unique().tolist()) != [False]:
        issues.append("imbalance_scope_mismatch")
    if sorted(export["mari_energy_included"].dropna().unique().tolist()) != [False]:
        issues.append("mari_scope_mismatch")
    if sorted(export["afrr_included"].dropna().unique().tolist()) != [False]:
        issues.append("afrr_scope_mismatch")
    ...
    if not comparison["acceptance_threshold_price"].round(12).equals(comparison["threshold_price_scenario"].round(12)):
    ...
    return {
        ...
        "primary_models": sorted(export["average_price_model_primary"].dropna().unique().tolist()),
        "comparator_models": sorted(export["average_price_model_comparator"].dropna().unique().tolist()),
    }
