...
from hourly_da.models.prophet_model import ProphetModel, ProphetSettings
from hourly_da.models.registry import build_naive_baseline_models


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the hourly DA naive + Prophet benchmark.")
    ...
    parser.add_argument("--holidays-prior-scale", type=float, default=10.0)
...
def main() -> None:
    ...
    run_id, overall_metrics, _, timing_summary, official_naive = run_benchmark_suite(
        ...
        models=models,
        show_progress=True,
        progress_label="prophet_benchmark",
    )
    ...
    prophet_timing = timing_summary[timing_summary["model"].str.startswith("prophet_")]
    if not prophet_timing.empty:
        print(prophet_timing[["model", "fit_time_mean_sec", "fit_time_warning_count"]].to_string(index=False))
