...
class ARIMASettings:
    ...
    min_observed_history_hours: int = 24 * 20
    maxiter: int = 10


class ARIMAModel(ForecastModel):
    def __init__(self, settings: ARIMASettings | None = None):
        super().__init__(name="arima", family="arima", fs_level="FS1")
        self.settings = settings or ARIMASettings()
    ...
    def fit(
            ...
            self._set_runtime_info(
                {
                    ...
                    "warning_counts_json": "{}",
                    "settings_json": json.dumps({"order": list(self.settings.order), "maxiter": self.settings.maxiter}),
                }
            )
            return
        ...
        try:
            with warnings.catch_warnings(record=True) as records:
                ...
                self._fitted_model = model.fit(method_kwargs={"maxiter": self.settings.maxiter})
