...
from steel.s4_4c6_deterministic_qh_full_year import CONFIG_PATH  # noqa: E402


def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run the governed native-QH C0/C1 price-insensitive and "
            "perfect-foresight calendar year through the shared annual engine."
        )
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Materialise and audit the 35,040-QH price/calendar input only.",
    )
    parser.add_argument(
        "--feasibility-shakedown",
        action="store_true",
        help=(
            "Run only one flat-price causal shakedown trajectory for C0 and C1; "
    ...
    parser.add_argument(
        ...
        type=int,
        help="Testing-only checkpoint stop across the selected trajectories.",
    )
    parser.add_argument(
        "--stop-after-day-per-case",
        type=int,
        help=(
            "Checkpoint every selected trajectory after this calendar day; "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "The unfiltered governed year run executes C0 and C1."
        ),
    )
